...
하지만 \`P@0@\`는 5개 \`P@2@\`는 6개를 더 필요로 하는데 남느 자원이 4개 밖에 없는 상황이 온다. 이러한 것을 *불안전한 할당(Unsafe allocation)*이라 한다.

운영체제는 자원을 할당 할 때 *불안전 할당*이 일어나지 않도록 해야한다. 이는 교착상태로 이어지기 때문이다.

이러한 문제를 *Banker's Algorithm*이라 한다.

### 교착상태 검출 및 복구 (Deadlock Detection & Recovery)

교착상태가 발생하는 것을 허용하는 방식이다. 프로세스가 필요한데로 자원을 다 나누어준다. 교착상태가 발생하게 되면 프로세스는 작업을 할 수 없게 된다.

그래서 os가 주기적으로 검사하도록 하게 되며, 대신에 검사하는 *overhead*가 큰 단점을 가진다.

교착상태를 발견(발생)하게 되면 복구를 한다. 복구하기 위해서는 일반적으로 주기적으로 기억을 해두어야 한다. 즉, 교착상태가 일어나기 전으로 돌리는 것이다.

프로세스 하나를 강제로 종료시키는 방법도 있지만 불가능한 경우도 있다. 그 때는 강제로 자원을 빼앗아 다른 프로세스에 주는 방법도 있다.

### 교착상태 무시

교착상태 무시는 교착상태를 해결하는 방법은 아니다. 실제로는 교착상태가 잘 안 일어나기도 하고, 4가지 교착상태의 조건에 있다 해도 무조건 발생하는 것은 아니라는 것이다.

다른 해결방법을 적용해서 성능이 오히려 떨어지기 때문에 PC 같은 경우에서 교착상태를 무시하는 방법도 가능하다.
`],
    ref: [
        {
