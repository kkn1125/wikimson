...
\`CPU\`가 하나의 *프로세스를 실행 중*일 때, \`프로세스\`가 *아직 작업 중*이고, *I/O를 사용하지도 않지만*, *강제*로 Ready Queue로 쫓겨나 *새로운 프로세스를 실행*하는 방식을 말한다.

병원으로 예를 들 때, \`환자\`(프로세스)를 진료보다가 \`응급환자\`(Ready Queue 중 급한 하나의 프로세스)를 먼저 보는 것과 유사하다. 즉, 현재 CPU 점유 중인 프로세스가 쫓겨나는 것을 허용하는 스케쥴링 인 것이다.
