...
|구분|설명|
|---|---|
|프로세스|프로그램을 구동하여 프로그램 자체와 프로그램의 상태가 메모리 상에서 실행되는 작업 단위를 말합니다. 메모리에 올라와 실행되고 있는 프로그램의 인스턴스(독립적 개체)라고 할 수 있고, CPU 자원의 할당을 받을 수 있고, 프로세스에 할당되는 메모리에는 스택, 힙, 데이터, 코드 영역을 포함합니다.|
...
|구분|설명|
|---|---|
|멀티 프로세스|멀티프로세스는 하나의 프로그램을 여러개의 프로세스로 구성하여 각 프로세스가 하나의 작업을 처리하는 것 입니다.
|^^|하나의 프로세스가 잘못 되어도 프로그램은 동작 합니다.|
|^^|<a class="ref" href="#cs-contextswitching" scroll-to="정의-1">ContextSwitching</a> 비용이 발생합니다.|
|멀티 스레드|프로그램을 여러 개의 스레드로 구성하고 각 스레드가 작업을 처리하는 것 입니다.|
|^^|시스템 자원 소모와 처리비용가 감소하고 스레드 간 자원 공유가 가능합니다.|
|^^|디버깅이 어렵고 동기화 이슈를 안고 있으며 하나의 스레드 오류로 전체 프로세스에 문제가 발생합니다.|
|^^|<a class="ref" href="#cs-deadlock" scroll-to="정의-1">교착상태</a> 가 발생하지 않도록 주의해야합니다.|
